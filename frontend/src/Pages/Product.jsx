...
const Product = () => {
  ...
  const {products,currency,exchangeRate,addToCart,darkMode}=useContext(ShopContext);
    ...
    <div className='border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100'>
      ...
      <div className='flex gap-12 sm:gap-12 flex-col sm:flex-row'>
        ...
        <div className='flex-1'>
              ...
              <p className='mt-5 text-3xl font-medium dark:text-white'>{currency}{productData.price*exchangeRate}</p>
              ...
              <div className='flex flex-col gap-4 my-8'>
                  ...
                  <div className='flex gap-2'>
                      {productData.sizes.map((item,index)=>(
                        <button onClick={()=>setSize(item)} className={`border py-3 px-4 bg-gray-100 ${item===size ?'border-orange-500':''}`} key={index}>
                          {item}
                        </button>
                      ))}
                  </div>
      ...
      <div className='mt-20 dark:text-white'>
          ...
          <div className='flex flex-col gap-4 border px-6 py-6 text-sm text-gray-500 dark:text-white'>
                ...
                <p>E-commerce (electronic commerce) is the activity of electronically buying or selling products and services on online platforms or over the Internet.</p>
          </div>
      </div>
